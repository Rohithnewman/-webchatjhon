/**
 * oil-motion — vendored verbatim from the installed `frontend-design` skill
 * (`assets/interactive-motion.ts`, upstream: oil-oil/oil-motion).
 *
 * Vendored rather than reimplemented: this project has no npm dependency for
 * it, and the skill ships it as a source asset. Do not edit this file — any
 * adaptation belongs in the wrappers beside it (`frame-loop.ts`, `timeline.ts`)
 * so the utility can be re-synced from the skill without merge pain.
 *
 * What it gives us:
 *   createFrameAnimator     spring/smoothDamp-driven frame position. `setTarget`
 *                           eases toward a frame (shortest path when circular),
 *                           `getCurrentFrame()` returns the FRACTIONAL position,
 *                           and `render(frame)` fires only when the rounded
 *                           frame changes — which is the sprite-sheet hook.
 *   createCssSpriteRenderer maps a frame index onto a sprite sheet's
 *                           background-position.
 */
