...
export const NODE_CATALOG: NodeDefinition[] = [
  ...
  { type: "llm", label: "AI response", category: "Conversation", icon: Bot, accent: "violet", defaults: { label: "AI response", prompt: "Respond to the user", model: "Configured in Phase 3", temperature: 0.7 }, fields: [{ key: "label", label: "Label" }, { key: "prompt", label: "System prompt", kind: "textarea" }, { key: "model", label: "Model" }, { key: "temperature", label: "Temperature", kind: "number" }] },
  ...
  { type: "knowledge_search", label: "Knowledge", category: "Logic", icon: Search, accent: "blue", defaults: { label: "Knowledge search", query: "{{last_message}}", topK: 5 }, fields: [{ key: "label", label: "Label" }, { key: "query", label: "Query" }, { key: "topK", label: "Top results", kind: "number" }] },
